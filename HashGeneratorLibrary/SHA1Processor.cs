...
namespace HashGeneratorLibrary
{
    ...
    /// </summary>
    public class SHA1Processor : ICryptoProcessor
    {
        /// <summary>
        ...
        public async Task<string> CalculateHash(Stream dataStream, bool useByteSeperator = false)
        {
            ...
            using (var sha1 = SHA1.Create())
            {
                using (dataStream)
                {
                    var hash = await sha1.ComputeHashAsync(dataStream);
                    output = BitConverter.ToString(hash);
                }
            }
        ...
        /// <exception cref="ArgumentNullException">Throws when SHA1 hash or datatStream is not set.</exception>
        ...
        /// <param name="buffer">Data buffer array.</param>
        /// <returns>Return true if the stream data matches with given SHA1 checksum.</returns>
        /// <exception cref="ArgumentNullException">Throws when SHA1 hash or buffer is not set.</exception>
